...
const project=createProjectFromLegacyEditorState({videoPath:'/tmp/p.mp4',originalVideoPath:'/tmp/o.mov',durationSeconds:5,projectDurationSeconds:5,zoomRegions:[],trimRegions:[],annotationRegions:[],audioRegions:[music],subtitleRegions:generated,cursorData:[],cursorSize:1,cursorSmoothing:true,showVectorCursor:true,cursorOffset:0,cropRegion:{x:0,y:0,width:1,height:1},wallpaper:'',shadowIntensity:0,showBlur:false,motionBlurEnabled:false,borderRadius:0,padding:0,aspectRatio:'16:9',exportQuality:'good'})
...
for(const token of ['Add to Audio Track','Transcribe','Split','Merge next','fontFamily','backgroundColor']) assert.ok(panel.includes(token))
