...
assert.match(editor, /source: "saved project", events: Array\.isArray\(cursorData\)/); assert.match(editor, /candidate\.regions\.length > best\.regions\.length/); assert.match(editor, /Auto Focus is unavailable for this recording/); assert.match(editor, /automatic focus cannot be reconstructed/);
...
console.log(JSON.stringify({ status: 'ok', audits: [30,31,32,33,34,35], focusDepths: 6, instant: true, portableClipboard: true, projectAutoFocus: 'manual-generate-only', telemetryFallback: 'best-generating-source' }));
