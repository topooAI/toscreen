...
try {
  ...
  assert.match(homeStyles, /\.coverScene[\s\S]*?transform-origin:\s*30% 52%/, 'side-camera projection anchors the located content at the near plane')
  ...
  assert.match(homeStyles, /\.coverDepthSoft::before\s*\{[\s\S]*?height:\s*36%[\s\S]*?blur\(\.7px\)/, 'soft depth layer begins the card-space focus falloff')
  assert.match(homeStyles, /\.coverDepthMedium::before\s*\{[\s\S]*?height:\s*27%[\s\S]*?blur\(1\.5px\)/, 'medium depth layer continues the focus falloff')
  assert.match(homeStyles, /\.coverDepthStrong::before\s*\{[\s\S]*?height:\s*17%[\s\S]*?blur\(2\.8px\)/, 'strong depth layer is restricted to the outer edge')
  assert.doesNotMatch(homeStyles, /\.coverDepth(?:Soft|Medium|Strong)::after/, 'optical blur never covers the title-bar edge')
  assert.match(homeStyles, /\.coverStage[\s\S]*?perspective:\s*460px[\s\S]*?perspective-origin:\s*28% 46%/, 'cover stage provides an offset side-camera perspective')
