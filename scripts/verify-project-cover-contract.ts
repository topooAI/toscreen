...
try {
  ...
  assert.match(homeStyles, /\.coverScene[\s\S]*?transform-origin:\s*50% 55%/, 'elevated camera projection anchors the content below the optical center')
  ...
  assert.match(homeStyles, /\.coverStage[\s\S]*?perspective:\s*900px[\s\S]*?perspective-origin:\s*50% 44%/, 'cover stage uses a restrained elevated-camera perspective')
  assert.match(homeStyles, /--scene-pitch:\s*48deg[\s\S]*?--scene-yaw:\s*-5deg/, 'cover angle is driven by top-down pitch with only minor side yaw')
  assert.match(homeStyles, /\.coverScene[\s\S]*?rotateX\(var\(--scene-pitch\)\)[\s\S]*?rotateY\(var\(--scene-yaw\)\)[\s\S]*?rotateZ\(var\(--scene-roll\)\)/, 'cover plane uses pitch, minor yaw, and roll instead of a flat rotation')
