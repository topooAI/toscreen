...
try {
  ...
  assert.match(homeStyles, /\.coverScene[\s\S]*?transform-origin:\s*70% 52%/, 'reversed side-camera projection anchors the located content at the right near plane')
  ...
  assert.match(homeStyles, /\.coverDepthSoft::before\s*\{[\s\S]*?width:\s*46%[\s\S]*?blur\(\.7px\)/, 'soft depth layer begins at the far side of the screen plane')
  assert.match(homeStyles, /\.coverDepthMedium::before\s*\{[\s\S]*?width:\s*32%[\s\S]*?blur\(1\.5px\)/, 'medium depth layer follows the screen depth axis')
  assert.match(homeStyles, /\.coverDepthStrong::before\s*\{[\s\S]*?width:\s*18%[\s\S]*?blur\(2\.8px\)/, 'strong depth layer is restricted to the far edge')
  assert.doesNotMatch(homeStyles, /\.coverDepth(?:Soft|Medium|Strong)::after/, 'optical blur never covers the title-bar edge')
  assert.match(homeStyles, /\.coverStage[\s\S]*?perspective:\s*460px[\s\S]*?perspective-origin:\s*72% 46%/, 'cover stage provides the reversed offset side-camera perspective')
  assert.match(homeStyles, /\.coverScene[\s\S]*?rotateX\(var\(--scene-pitch\)\)[\s\S]*?rotateY\(var\(--scene-yaw\)\)[\s\S]*?rotateZ\(var\(--scene-roll\)\)/, 'cover plane uses pitch, side yaw, and roll instead of a flat rotation')
