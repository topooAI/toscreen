...
expect('window ID reaches native recorder', handlers.includes("selectedSource.id.split(':')[1]") && native.includes('windowId: options?.windowId'))
expect('custom area reaches native recorder', launch.includes('captureArea: selectedType === "Area" ? area') && native.includes('captureArea: options?.captureArea'))
expect('microphone device and switches are real inputs', launch.includes('enumerateDevices()') && launch.includes('audioDeviceId: microphoneId') && native.includes('audioDeviceId: options?.audioDeviceId'))
expect('microphone meter uses Web Audio analyser', launch.includes('createAnalyser()') && launch.includes('getByteFrequencyData'))
expect('system audio is configurable', launch.includes('includeSystemAudio: systemAudioOn') && native.includes('includeSystemAudio: options?.includeSystemAudio'))
