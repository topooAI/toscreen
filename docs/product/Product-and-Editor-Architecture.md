...
| PH1-60 Timeline 可视分轨门禁 / Timeline visual lane-wrapping gate | ✅ Done | `TimelineEditor` 现在复用 `partitionIntoTimelineLanes` 给 Zoom/Focus、Annotation、Audio 做可视分轨；新增 `npm run audit:timeline-lane-wrapping`，验证重叠片段不会落在同一可视 lane，贴边片段可以复用同一 lane。 / `TimelineEditor` now uses `partitionIntoTimelineLanes` for Zoom/Focus, annotation, and audio visual lane partitioning; added `npm run audit:timeline-lane-wrapping` to verify overlapping clips do not share the same visual lane while edge-touching clips can reuse a lane. |
