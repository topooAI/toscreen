...
| PH1-18 导出路径接入渲染合同 / Export path render-contract adoption | ✅ Done | `VideoEditor` 导出时会先从当前状态生成 ProjectModel，再通过 `getProjectRenderSettings` 读取 canvas、zoom、trim、annotation、cursor、motion blur 和 export quality；音频仍保留内存态 `audioRegions`，避免丢失未落盘 `File`。 / `VideoEditor` export now builds a ProjectModel from current state and reads canvas, zoom, trim, annotation, cursor, motion blur, and export quality through `getProjectRenderSettings`; audio keeps in-memory `audioRegions` to preserve unsaved `File` objects. |
...
| PH1-21 Phase 1 聚合审计 / Phase 1 aggregate audit | ✅ Done | 已新增 `npm run audit:phase1`，统一执行 TypeScript 门禁、Phase 1 audit registry、ProjectModel smoke、ProjectModel 恢复审计、ProjectModel roundtrip 审计、真实录制恢复审计、Preview/Export 渲染合同审计、导出入口审计、未来模型入口审计、多源画面模型审计、Camera 模型审计、Track/Clip 兼容性审计、lane wrapping 审计、Track 层级审计、Asset/Clip 兼容性审计、AI Edit Plan 结构审计、ProjectModel 用户评审包审计、sidecar/legacy parity 审计和 ProjectModel 评审文档审计。 / Added `npm run audit:phase1` to run TypeScript, Phase 1 audit registry, ProjectModel smoke, ProjectModel restore, ProjectModel roundtrip, real recording restore, preview/export render contract, export entrypoint, future model entry, multi-source model, Camera model, track/clip compatibility, lane wrapping, track hierarchy, asset/clip compatibility, AI Edit Plan structure, ProjectModel review packet, sidecar/legacy parity, and ProjectModel review doc audits together. |
...
| PH1-33 用户模型确认 / User model review | ⏳ Pending | 需要用户确认 Project / Asset / Track / Clip / UI Source / AI Edit Plan 方向是否符合未来产品。 / User should confirm whether the Project / Asset / Track / Clip / UI Source / AI Edit Plan direction matches the product vision. |
