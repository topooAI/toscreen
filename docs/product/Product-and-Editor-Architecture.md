...
| PH1-20 Phase 1 聚合审计 / Phase 1 aggregate audit | ✅ Done | 已新增 `npm run audit:phase1`，统一执行 TypeScript 门禁、真实录制恢复审计、Preview/Export 渲染合同审计、导出入口审计和未来模型入口审计。 / Added `npm run audit:phase1` to run TypeScript, real recording restore, preview/export render contract, export entrypoint, and future model entry audits together. |
