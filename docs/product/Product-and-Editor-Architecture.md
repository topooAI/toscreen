...
| PH1-60 Timeline 可视分轨门禁 / Timeline visual lane-wrapping gate | ✅ Done | `TimelineEditor` 现在复用 `partitionIntoTimelineLanes` 给 Annotation/Audio 做可视分轨；新增 `npm run audit:timeline-lane-wrapping`，验证重叠 Annotation 不会落在同一可视 lane，贴边片段可以复用同一 lane。 / `TimelineEditor` now uses `partitionIntoTimelineLanes` for Annotation/Audio visual lane partitioning; added `npm run audit:timeline-lane-wrapping` to verify overlapping annotations do not share the same visual lane while edge-touching clips can reuse a lane. |
...
- Timeline 可视分轨门禁更新：新增 `audit:timeline-lane-wrapping`，验证 Annotation/Audio 在时间轴可视层使用同一分轨 helper，重叠 Annotation 自动换到不同可视 lane。
  Timeline visual lane-wrapping gate update: added `audit:timeline-lane-wrapping` to verify Annotation/Audio use the same timeline visual lane helper and overlapping annotations automatically move to separate visual lanes.
