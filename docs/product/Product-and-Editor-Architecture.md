...
- Phase 1 handoff 更新：新增 `audit:phase1-handoff`，把最新真实录制、ProjectModel 恢复摘要、旧无 Scene sidecar 的 `sceneMigration`、待验收 UA 和 Electron 测试入口输出成一个预检包，便于用户开始实机验收。
  Phase 1 handoff update: added `audit:phase1-handoff`, producing a preflight packet with the latest real recording, ProjectModel restore summary, `sceneMigration` for old scene-less sidecars, pending UA items, and Electron test entry points for hands-on acceptance.
