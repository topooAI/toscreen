...
| PH1-16 Preview/Export 渲染合同 / Preview/export render contract | ✅ Done | 已新增 `getProjectRenderSettings` 和 `npm run audit:preview-export-contract`，验证同一份 ProjectModel 能恢复画布、时间轴、光标、动效和导出质量等共同渲染参数。 / Added `getProjectRenderSettings` and `npm run audit:preview-export-contract` to verify that one ProjectModel restores shared canvas, timeline, cursor, effects, and export-quality render settings. |
| PH1-17 导出路径接入渲染合同 / Export path render-contract adoption | ✅ Done | `VideoEditor` 导出时会先从当前状态生成 ProjectModel，再通过 `getProjectRenderSettings` 读取 canvas、zoom、trim、annotation、cursor、motion blur 和 export quality；音频仍保留内存态 `audioRegions`，避免丢失未落盘 `File`。 / `VideoEditor` export now builds a ProjectModel from current state and reads canvas, zoom, trim, annotation, cursor, motion blur, and export quality through `getProjectRenderSettings`; audio keeps in-memory `audioRegions` to preserve unsaved `File` objects. |
| PH1-18 预览路径接入渲染合同 / Preview path render-contract adoption | ✅ Done | `VideoPlayback` 现在使用同一份 `currentRenderSettings` 读取 aspect ratio、wallpaper、zoom、trim、annotation、cursor、blur、shadow、padding 和 crop，减少预览与导出参数分叉。 / `VideoPlayback` now uses the same `currentRenderSettings` for aspect ratio, wallpaper, zoom, trim, annotation, cursor, blur, shadow, padding, and crop, reducing preview/export parameter drift. |
| PH1-19 导出入口收口 / Export entrypoint consolidation | ✅ Done | 已删除未使用的 `useVideoExport` 旁路 hook，并新增 `npm run audit:export-entrypoints`，确保 `VideoExporter` 只从主 `VideoEditor` 的 render settings 路径进入。 / Removed the unused `useVideoExport` bypass hook and added `npm run audit:export-entrypoints`, ensuring `VideoExporter` is only entered through the main `VideoEditor` render-settings path. |
| PH1-20 Phase 1 聚合审计 / Phase 1 aggregate audit | ✅ Done | 已新增 `npm run audit:phase1`，统一执行 TypeScript 门禁、ProjectModel 恢复审计、真实录制恢复审计、Preview/Export 渲染合同审计、导出入口审计和未来模型入口审计。 / Added `npm run audit:phase1` to run TypeScript, ProjectModel restore, real recording restore, preview/export render contract, export entrypoint, and future model entry audits together. |
| PH1-21 用户模型确认 / User model review | ⏳ Pending | 需要用户确认 Project / Asset / Track / Clip / UI Source / AI Edit Plan 方向是否符合未来产品。 / User should confirm whether the Project / Asset / Track / Clip / UI Source / AI Edit Plan direction matches the product vision. |
