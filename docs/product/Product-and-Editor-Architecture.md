...
| PH1-21 Phase 1 聚合审计 / Phase 1 aggregate audit | ✅ Done | 已新增 `npm run audit:phase1`，统一执行 TypeScript 门禁、Phase 1 audit registry、ProjectModel smoke、ProjectModel 恢复审计、ProjectModel roundtrip 审计、真实录制恢复审计、Preview/Export 渲染合同审计、工程总长审计、导出工程总长审计、Timeline 时长域审计、预览工程时间审计、导出黑屏尾部渲染审计、Screen Studio 核心体验合同审计、Screen Studio 控制入口审计、导出入口审计、未来模型入口审计、多源画面模型审计、Camera 模型审计、Camera migration 审计、Track/Clip 兼容性审计、lane wrapping 审计、Clip 重叠策略审计、Track 层级审计、Asset/Clip 兼容性审计、核心 Clip 合同审计、Annotation 合同审计、Motion Clip 合同审计、AI Edit Plan 结构审计、AI Edit Plan 生命周期审计、ProjectModel 用户评审包审计、sidecar/legacy parity 审计、Scene 合同审计、ProjectModel 评审文档审计、Phase 1 ownership list 审计、Phase 1 readiness 审计、Phase 1 acceptance state 审计、Phase 1 用户验收记录审计、Phase 1 handoff 审计、导出音频 render settings 审计和预览音频 render settings 审计。 / Added `npm run audit:phase1` to run TypeScript, Phase 1 audit registry, ProjectModel smoke, ProjectModel restore, ProjectModel roundtrip, real recording restore, preview/export render contract, project-duration contract, export-duration render-settings contract, timeline duration-domain audit, preview project-time audit, export black-tail rendering audit, Screen Studio core contract, Screen Studio control wiring audit, export entrypoint, future model entry, multi-source model, Camera model, Camera migration, track/clip compatibility, lane wrapping, clip-overlap policy, track hierarchy, asset/clip compatibility, core clip contract, annotation contract, motion clip contract, AI Edit Plan structure, AI Edit Plan lifecycle, ProjectModel review packet, sidecar/legacy parity, scene contract, ProjectModel review doc, Phase 1 ownership list, Phase 1 readiness, Phase 1 acceptance state, Phase 1 user acceptance record, Phase 1 handoff, export-audio render settings, and preview-audio render settings audits together. |
...
| PH1-63 Timeline 点击映射门禁 / Timeline seek mapping gate | ✅ Done | `TimelineEditor` 现在通过 `resolveTimelineSeekFromClientX` 统一时间轴点击和播放指针拖拽的坐标映射；新增 `npm run audit:timeline-seek-mapping`，验证左侧呼吸区归零、trackStartPx 起点、range offset、工程时长 clamp，以及 Trim 折叠时 effective time 到 source time 的映射。 / `TimelineEditor` now routes timeline click seek and playhead drag through `resolveTimelineSeekFromClientX`; added `npm run audit:timeline-seek-mapping` to verify left breathing-area reset, trackStartPx origin, range offset, project-duration clamp, and Trim-folded effective-to-source mapping. |
| PH1-64 Timeline 播放指针时间源门禁 / Timeline playhead time-source gate | ✅ Done | `PlaybackCursor` 现在通过 `resolveTimelinePlayheadDisplayTime` 选择显示时间源；新增 `npm run audit:timeline-playhead-time`，验证播放时可跟随 video.currentTime，暂停/拖拽/冻结外部时间/无效 external time 时回到 React currentTime，并保护 Trim 折叠 source->effective 映射。 / `PlaybackCursor` now selects its display time source through `resolveTimelinePlayheadDisplayTime`; added `npm run audit:timeline-playhead-time` to verify playback may follow video.currentTime, paused/dragging/frozen/invalid external time falls back to React currentTime, and folded Trim source-to-effective mapping stays guarded. |
| PH1-65 Timeline 轨道起点门禁 / Timeline track-origin gate | ✅ Done | `Row` 和 `TimelineEditor` 现在共用 `timelineTrackOrigin.ts` 的 Sidebar 宽度、16px 呼吸留白和 fallback trackStartPx；新增 `npm run audit:timeline-track-origin`，验证真实 track area 测量和 Axis/Cursor/Seek 共用同一个 trackStartPx。 / `Row` and `TimelineEditor` now share sidebar width, the 16px breathing gap, and fallback trackStartPx from `timelineTrackOrigin.ts`; added `npm run audit:timeline-track-origin` to verify real track-area measurement and shared Axis/Cursor/Seek trackStartPx wiring. |
| PH1-65A 无限 Timeline 缩放与平移门禁 / Infinite timeline zoom and pan gate | ✅ Done | `TimelineWrapper` 通过 `timelineRangeZoom.ts` 处理 0 点边界和左对齐 zoom；`npm run audit:timeline-range-zoom` 验证缩放固定左边缘、滚轮平移保留请求起点，并且右侧工作区不受 Main Clip 或项目内容时长封顶。 / `TimelineWrapper` handles the zero boundary and left-aligned zoom through `timelineRangeZoom.ts`; `npm run audit:timeline-range-zoom` verifies a stable left edge during zoom, direct wheel pan, and a right workspace that is not capped by Main Clip or project content duration. |
| PH1-66 Timeline 音频源边界门禁 / Timeline audio source-bound resize gate | ✅ Done | 音频波形偏移、左侧源起点墙、右侧源结尾墙和松手后的 audio span clamp 现在共用 `timelineAudioResizeBounds.ts`；新增 `npm run audit:audio-resize-bounds`，把 AUDIO-02A/03A/04A 作为机器合同纳入 Phase 1。 / Audio waveform offset, left source-start wall, right source-end wall, and committed audio span clamp now share `timelineAudioResizeBounds.ts`; added `npm run audit:audio-resize-bounds`, registering AUDIO-02A/03A/04A as Phase 1 machine contracts. |
| PH1-67 Main Clip 分段门禁 / Main Clip segmentation gate | ✅ Done | `TimelineEditor` 现在通过 `timelineMainClipSegments.ts` 生成 Trim 折叠后的主视频片段；新增 `npm run audit:main-clip-segmentation`，验证多段、嵌套、相交和越界 Trim 下主轨只显示保留源片段。 / `TimelineEditor` now builds Trim-folded main-video clips through `timelineMainClipSegments.ts`; added `npm run audit:main-clip-segmentation` to verify the main track shows only kept source ranges under multiple, nested, overlapping, and out-of-range Trim regions. |
| PH1-68 原声音频手风琴挂载门禁 / Original audio accordion gate | ✅ Done | `TimelineEditor` 现在通过 `timelineOriginalAudio.ts` 区分未分离原声、分离原声和普通音频；新增 `npm run audit:original-audio-accordion`，验证未分离原声挂载到视频片段、分离原声保留为独立音轨，并保护 Trim 折叠主片段的原声 source range 同步。 / `TimelineEditor` now uses `timelineOriginalAudio.ts` to distinguish non-detached original audio, detached original audio, and normal audio; added `npm run audit:original-audio-accordion` to verify non-detached original audio attaches to video clips, detached original audio remains standalone, and Trim-folded Main Clip segments keep matching original-audio source ranges. |
| PH1-69 主视频缩略图分段门禁 / Main video thumbnail segmentation gate | ✅ Done | `VideoThumbnails` 现在通过 `timelineThumbnailSegments.ts` 生成默认/Zoom/Focus 缩略图分段和边界百分比；新增 `npm run audit:main-video-thumbnails`，验证分割虚线与缩略图分段共用同一源时间边界，并支持 Focus resize preview 边界同步。 / `VideoThumbnails` now builds default/Zoom/Focus thumbnail segments and boundary percentages through `timelineThumbnailSegments.ts`; added `npm run audit:main-video-thumbnails` to verify separator lines and thumbnail segments share source-time boundaries and support Focus resize-preview boundary sync. |
| PH1-70 时间轴调试信号门禁 / Timeline debug-signal gate | ✅ Done | 新增 `npm run audit:timeline-debug-signal`，确保点击时间轴的 `[TimelineSeek]` 调试日志仍经过 `resolveTimelineSeekFromClientX` 输出 raw/effective/source 时间，并在 resize 期间拦截合成 seek；DEV-02B 仍需 Electron DevTools 实机确认。 / Added `npm run audit:timeline-debug-signal` to ensure timeline-click `[TimelineSeek]` debug logs still route through `resolveTimelineSeekFromClientX`, emit raw/effective/source time, and block synthetic seek while resizing; DEV-02B still needs Electron DevTools hands-on confirmation. |
| PH1-71 统一拉伸把手门禁 / Unified resize-handle gate | ✅ Done | 新增 `npm run audit:timeline-resize-handles`，保护所有 clip 类型复用竖向白色圆角小手柄，并禁止旧 SVG 端帽、横线、黄色把手和文本符号回归；同时修正时间轴验收文档里的旧过时判断。 / Added `npm run audit:timeline-resize-handles` to guard the shared vertical white rounded resize handles across clip types and prevent legacy SVG caps, horizontal dashes, yellow handles, and text glyphs from returning; also corrected the stale timeline acceptance claim. |
| PH1-72 片段基础样式门禁 / Base clip style gate | ✅ Done | 新增 `npm run audit:timeline-clip-style`，保护 CLIP-01 到 CLIP-05 的 3px 微缝、标题左上角 6px 对齐、纯文本无 Lucide 图标、6px 圆角和内发光选中态，并将这些样式项从单纯人工验收升级为机器门禁保护。 / Added `npm run audit:timeline-clip-style` to guard CLIP-01 through CLIP-05: the 3px gap, 6px top-left title alignment, text-only labels without Lucide icons, 6px radius, and inset selected glow, upgrading these style items from manual-only acceptance to machine-guarded contracts. |
| PH1-73 音频波形底部布局门禁 / Audio waveform bottom-layout gate | ✅ Done | 新增 `npm run audit:timeline-waveform-layout`，保护 AUDIO-01 的波形底部 45%、标题左上角层级、source-time SVG 对齐和选中态音量包络下半区规则，避免后续样式改动遮挡音频标题。 / Added `npm run audit:timeline-waveform-layout` to guard AUDIO-01 waveform bottom 45%, top-left title layering, source-time SVG alignment, and selected volume-envelope lower-half rules, preventing future style changes from covering audio labels. |
| PH1-74 物理 Trim 轨隐藏门禁 / Physical Trim-row hidden gate | ✅ Done | 新增 `npm run audit:timeline-trim-row-hidden`，保护 VIDEO-02 的独立 Trim Row 不再渲染、Trim item 嵌套挂载到 Main Track、主片段分段继续通过 `buildMainClipSegments` 生成。 / Added `npm run audit:timeline-trim-row-hidden` to guard VIDEO-02: standalone Trim Row stays hidden, Trim items mount inside Main Track, and main-clip segmentation continues through `buildMainClipSegments`. |
| PH1-75 播放控件工程总长门禁 / Playback controls project-duration gate | ✅ Done | 扩展 `npm run audit:timeline-duration-domains`，保护 `VideoEditor` 将 `projectDuration` 传给 `TimelineEditor`，`sourceVideoDuration` 只保留给源视频/Trim，且 `PlaybackControls` 显示和步进 clamp 使用工程总长。 / Expanded `npm run audit:timeline-duration-domains` to guard `VideoEditor` passing `projectDuration` to `TimelineEditor`, keeping `sourceVideoDuration` only for source/Trim operations, and ensuring `PlaybackControls` display and forward-seek clamp use project duration. |
| PH1-76 Screen Studio 控制入口门禁 / Screen Studio control wiring gate | ✅ Done | 新增 `npm run audit:screenstudio-control-wiring`，保护右侧控制面板到核心体验的连接：Zoom 深度、Focus 位置拖拽、Auto-Zoom、背景虚化、Motion Blur、Padding、圆角、阴影、系统光标样式、光标大小和同步偏移。 / Added `npm run audit:screenstudio-control-wiring` to guard the right sidebar controls for core UX: Zoom depth, Focus-position dragging, Auto-Zoom, background blur, Motion Blur, padding, radius, shadow, system cursor style, cursor size, and sync offset. |
| PH1-77 ProjectModel 画布设置门禁 / ProjectModel canvas settings gate | ✅ Done | 新增 `npm run audit:project-model-canvas-settings`，让 validator 强校验 aspect ratio、wallpaper、blur、padding、radius、shadow、crop、export quality 和 legacy `motionBlurEnabled`，防止保存出结构合法但恢复/渲染字段不可用的项目。 / Added `npm run audit:project-model-canvas-settings`, making the validator enforce aspect ratio, wallpaper, blur, padding, radius, shadow, crop, export quality, and legacy `motionBlurEnabled` so a project cannot be structurally valid while restore/render fields are unusable. |
| PH1-78 ProjectModel 实体 schema 门禁 / ProjectModel entity-schema gate | ✅ Done | 新增 `npm run audit:project-model-entity-schema`，让 validator 强校验项目 id/name/timestamp、Asset/Track/Clip 名称和受控 type 枚举，防止 AI、导入器或迁移层写入未知实体类型。 / Added `npm run audit:project-model-entity-schema`, making the validator enforce project id/name/timestamps plus Asset/Track/Clip names and controlled type enums so AI, importers, or migration code cannot write unknown entity types. |
| PH1-79 UI Source schema 门禁 / UI Source schema gate | ✅ Done | 新增 `npm run audit:project-model-ui-source-schema`，让 validator 强校验 UI source provider、source/file path、capturedAt、element role、stable selector 和 bounds，为 UI-aware Motion Compiler 保留可信结构入口。 / Added `npm run audit:project-model-ui-source-schema`, making the validator enforce UI source provider, source/file path, capturedAt, element role, stable selector, and bounds so the UI-aware Motion Compiler has a trustworthy structured entry point. |
| PH1-80 Clip 源范围合同门禁 / Clip source-range contract gate | ✅ Done | 新增 `npm run audit:project-model-clip-source`，让 validator 强校验 Clip 的 `sourceStartMs/sourceEndMs` 成对、有限、非负且顺序正确，并限制 legacy 来源只能指向受控的 VideoEditor region。 / Added `npm run audit:project-model-clip-source`, making the validator enforce paired, finite, non-negative, ordered Clip `sourceStartMs/sourceEndMs` values and limiting legacy provenance to controlled VideoEditor region data. |
...
- Timeline 点击映射门禁更新：新增 `audit:timeline-seek-mapping`，把时间轴点击和播放指针拖拽坐标统一收敛到 `resolveTimelineSeekFromClientX`，保护左侧呼吸区归零、trackStartPx 起点、range offset、工程时长 clamp 和 Trim 折叠 source/effective 映射。
  Timeline seek-mapping gate update: added `audit:timeline-seek-mapping`, routing timeline click seek and playhead drag through `resolveTimelineSeekFromClientX` to guard left breathing-area reset, trackStartPx origin, range offset, project-duration clamp, and Trim-folded source/effective mapping.
- Timeline 播放指针时间源门禁更新：新增 `audit:timeline-playhead-time`，把 `PlaybackCursor` 的时间源选择收敛到 `resolveTimelinePlayheadDisplayTime`，防止暂停/拖拽时再次被 HTML video timeupdate 拉回。
  Timeline playhead time-source gate update: added `audit:timeline-playhead-time`, routing `PlaybackCursor` time-source selection through `resolveTimelinePlayheadDisplayTime` so paused/dragging states cannot be pulled back by HTML video timeupdate again.
- Timeline 音频源边界门禁更新：新增 `audit:audio-resize-bounds`，把音频左拉伸波形对齐、左侧源起点墙、右侧源结尾墙和提交后的 span clamp 收敛到同一个 helper；AUDIO-02B/03B/04B 仍需 Electron 实机验收。
  Timeline audio source-bound resize gate update: added `audit:audio-resize-bounds`, routing waveform alignment, left source-start wall, right source-end wall, and committed span clamp through one helper; AUDIO-02B/03B/04B still require hands-on Electron review.
- Timeline 原声音频手风琴门禁更新：新增 `audit:original-audio-accordion` 和 `timelineOriginalAudio.ts`，固定未分离原声挂载、分离原声下沉、Trim 折叠主片段原声 source range 同步这三个合同；AUDIO-05B 仍需 Electron 实机验收展开/折叠手感。
  Timeline original-audio accordion gate update: added `audit:original-audio-accordion` and `timelineOriginalAudio.ts`, locking the contracts for non-detached original attachment, detached-original standalone lanes, and Trim-folded Main Clip source-range sync; AUDIO-05B still needs Electron hands-on expand/collapse review.
- 主视频缩略图分段门禁更新：新增 `audit:main-video-thumbnails` 和 `timelineThumbnailSegments.ts`，固定主轨缩略图默认/Zoom/Focus 分段、虚线分割百分比和 Focus resize preview 边界同步；VIDEO-01B 仍需 Electron 真实缩略图渲染验收。
  Main video thumbnail segmentation gate update: added `audit:main-video-thumbnails` and `timelineThumbnailSegments.ts`, locking main-track default/Zoom/Focus thumbnail segmentation, dashed separator percentages, and Focus resize-preview boundary sync; VIDEO-01B still needs hands-on Electron thumbnail-render review.
- Main Clip 分段门禁更新：新增 `audit:main-clip-segmentation`，把 Trim 折叠后的主视频片段生成收敛到 `buildMainClipSegments`，防止嵌套/相交 Trim 让主轨显示被裁切源片段；VIDEO-03B 仍需 Electron 视觉验收。
  Main Clip segmentation gate update: added `audit:main-clip-segmentation`, routing Trim-folded main-video clip generation through `buildMainClipSegments` so nested/overlapping Trim regions cannot make the main track show trimmed source ranges; VIDEO-03B still needs hands-on Electron visual review.
- Timeline 轨道起点门禁更新：新增 `audit:timeline-track-origin`，把 Sidebar 宽度、16px 呼吸留白、fallback trackStartPx 和真实 track area 测量集中到 `timelineTrackOrigin.ts`，防止 Axis、Cursor 和 Seek 再次分叉。
  Timeline track-origin gate update: added `audit:timeline-track-origin`, centralizing sidebar width, the 16px breathing gap, fallback trackStartPx, and real track-area measurement in `timelineTrackOrigin.ts` so Axis, Cursor, and Seek cannot drift apart again.
- 无限 Timeline 门禁更新：`audit:timeline-range-zoom` 和 `timelineRangeZoom.ts` 固定缩放时的左边缘和 0 点边界，滚轮直接平移，并移除由 Main Clip 或项目内容时长推导出的右侧上限；VIDEO-04B 仍需 Electron 手感验收。
  Infinite timeline gate update: `audit:timeline-range-zoom` and `timelineRangeZoom.ts` keep the left edge and zero boundary stable during zoom, support direct wheel pan, and remove the right limit derived from Main Clip or project content duration; VIDEO-04B still needs Electron hands-on feel review.
- Timeline 调试信号门禁更新：新增 `audit:timeline-debug-signal`，把 `[TimelineSeek]` raw/effective/source 日志、统一 seek helper 接线和 resize 期间拦截合成 seek 纳入 Phase 1 聚合审计；DEV-02B 仍需 Electron DevTools 实机确认。
  Timeline debug-signal gate update: added `audit:timeline-debug-signal`, registering `[TimelineSeek]` raw/effective/source logs, unified seek-helper wiring, and resize-time synthetic seek blocking in the Phase 1 aggregate audit; DEV-02B still needs Electron DevTools hands-on confirmation.
- Timeline 统一拉伸把手门禁更新：新增 `audit:timeline-resize-handles`，保护 `Item.tsx`/`ItemGlass.module.css` 中所有 clip 类型复用竖向白色圆角小手柄，并防止旧 SVG 端帽、横线、黄色把手和文本符号回归。
  Timeline unified resize-handle gate update: added `audit:timeline-resize-handles`, guarding the shared vertical white rounded handles in `Item.tsx`/`ItemGlass.module.css` across clip types and preventing old SVG caps, horizontal dashes, yellow handles, and text glyphs from returning.
- Timeline 片段基础样式门禁更新：新增 `audit:timeline-clip-style`，保护 CLIP-01 到 CLIP-05 的 3px 微缝、标题左上角 6px 对齐、纯文本无 Lucide 图标、6px 圆角和内发光选中态。
  Timeline base clip style gate update: added `audit:timeline-clip-style`, guarding CLIP-01 through CLIP-05: the 3px gap, 6px top-left title alignment, text-only labels without Lucide icons, 6px radius, and inset selected glow.
- Timeline 音频波形布局门禁更新：新增 `audit:timeline-waveform-layout`，保护音频波形只显示在底部 45%、标题层级在波形之上、选中态音量包络限制在下半区。
  Timeline audio waveform layout gate update: added `audit:timeline-waveform-layout`, guarding audio waveforms in the bottom 45%, title layering above the waveform, and selected volume envelopes in the lower half.
- Timeline 物理 Trim 轨隐藏门禁更新：新增 `audit:timeline-trim-row-hidden`，保护 Trim 不再作为独立轨道渲染，而是嵌套挂载到 Main Track，并继续通过 `buildMainClipSegments` 展示裁切后的主片段。
  Timeline physical Trim-row hidden gate update: added `audit:timeline-trim-row-hidden`, guarding Trim against returning as a standalone row; Trim stays nested in Main Track and continues to use `buildMainClipSegments` for folded main clips.
- Timeline 播放控件工程总长门禁更新：扩展 `audit:timeline-duration-domains`，保护 PlaybackControls 的总时长和前进步进使用工程总长，而不是源视频长度。
  Timeline playback-controls project-duration gate update: expanded `audit:timeline-duration-domains` to guard PlaybackControls total duration and forward-step clamping against project duration rather than source-video duration.
- Screen Studio 控制入口门禁更新：新增 `audit:screenstudio-control-wiring`，保护 Zoom 深度、Focus 位置拖拽、Auto-Zoom、背景虚化、Motion Blur、布局和系统光标控制的 UI 接线，并让 Focus overlay 初始 pointer-events 与选中/播放状态一致。
  Screen Studio control-wiring gate update: added `audit:screenstudio-control-wiring` to guard UI wiring for Zoom depth, Focus-position dragging, Auto-Zoom, background blur, Motion Blur, layout, and system cursor controls, and aligned the Focus overlay initial pointer-events with selected/playback state.
- Phase 1 背景导出一致性门禁更新：新增 `npm run audit:export-background-parity`，保护 Image/Color/Gradient 背景在预览和导出之间保持同一数据入口，并让导出端按括号层级解析 CSS gradient，避免 `rgba(...)` 逗号导致渐变色标被拆坏。
  Phase 1 export-background parity gate update: added `npm run audit:export-background-parity` to keep Image/Color/Gradient backgrounds aligned between preview and export, and to parse CSS gradients by parenthesis depth so `rgba(...)` commas do not break gradient stops.
- Phase 1 真实录制素材文件门禁更新：`npm run audit:recordings` 现在会输出并校验最新真实项目中 ProjectModel 本地素材引用，新增 `npm run audit:recording-asset-files` 防止 Electron 重启恢复前置文件检查被移除。
  Phase 1 real-recording asset-file gate update: `npm run audit:recordings` now reports and verifies local ProjectModel asset references in the latest real project, and `npm run audit:recording-asset-files` prevents the preflight file check for Electron restart restore from being removed.
- Phase 1 验收入口素材证据更新：`npm run audit:phase1-handoff` 和 `npm run audit:phase1-user-review-packet` 现在都会输出 ProjectModel 本地素材文件检查 `assetFiles`，让 UA-02 重启恢复验收入口直接显示 checked/skipped/missing 文件状态。
  Phase 1 acceptance-entry asset evidence update: `npm run audit:phase1-handoff` and `npm run audit:phase1-user-review-packet` now report ProjectModel local asset-file check `assetFiles`, so the UA-02 restart-restore entry directly shows checked/skipped/missing file state.
- Phase 1 ProjectModel 画布设置门禁更新：新增 `npm run audit:project-model-canvas-settings`，把画布、背景、裁剪、导出质量和 legacy `motionBlurEnabled` 纳入 validator 与聚合审计，避免重启恢复或预览/导出链路读到不可渲染字段。
- Screen Studio 全功能集成门禁更新：`npm run audit:phase1` 现在额外执行 `npm run audit:recording-session`、`npm run audit:recording-session-contract`、`npm run audit:editing-session`、`npm run audit:editing-product-integration`、`npm run audit:editing-project-tail`、`npm run audit:presentation`、`npm run audit:presentation-ui`、`npm run audit:focus-30-35`、`npm run audit:presentation-recording-contract`、`npm run audit:audio-captions`、`npm run audit:projects-presets`、`npm run audit:ios-device-capture`、`npm run audit:export-share` 与 `npm run audit:share-worker`；新增能力不再游离于总门禁之外。
  Screen Studio full-feature integration gate update: `npm run audit:phase1` now also runs `npm run audit:recording-session`, `npm run audit:recording-session-contract`, `npm run audit:editing-session`, `npm run audit:editing-product-integration`, `npm run audit:editing-project-tail`, `npm run audit:presentation`, `npm run audit:presentation-ui`, `npm run audit:focus-30-35`, `npm run audit:presentation-recording-contract`, `npm run audit:audio-captions`, `npm run audit:projects-presets`, `npm run audit:ios-device-capture`, `npm run audit:export-share`, and `npm run audit:share-worker`, so newly integrated capabilities can no longer sit outside the aggregate gate.
  Phase 1 ProjectModel canvas-settings gate update: added `npm run audit:project-model-canvas-settings`, covering canvas, background, crop, export quality, and legacy `motionBlurEnabled` in the validator and aggregate audit so restart restore and preview/export paths cannot consume unrenderable fields.
- Phase 1 ProjectModel 实体 schema 门禁更新：新增 `npm run audit:project-model-entity-schema`，把项目元数据、Asset/Track/Clip 名称和受控类型枚举纳入 validator 与聚合审计，避免未来 AI 或导入器写出未知实体类型。
  Phase 1 ProjectModel entity-schema gate update: added `npm run audit:project-model-entity-schema`, covering project metadata plus Asset/Track/Clip names and controlled type enums in the validator and aggregate audit so future AI or importers cannot write unknown entity types.
- Phase 1 UI Source schema 门禁更新：新增 `npm run audit:project-model-ui-source-schema`，把 UI source provider、source/file path、capturedAt、element role、stable selector 和 bounds 纳入 validator 与聚合审计，为 UI-aware motion 和未来 UI 源文件衔接动画保留可验证结构。
  Phase 1 UI Source schema gate update: added `npm run audit:project-model-ui-source-schema`, covering UI source provider, source/file path, capturedAt, element role, stable selector, and bounds in the validator and aggregate audit so UI-aware motion and future UI-source-driven animation keep a verifiable structure.
- Phase 1 Clip 源范围合同门禁更新：新增 `npm run audit:project-model-clip-source`，把 Clip 的 `sourceStartMs/sourceEndMs` 成对约束、源范围顺序、`trackId/assetId/name` 运行时类型和 legacy VideoEditor region 来源纳入 validator 与聚合审计，避免保存恢复后时间轴 clip 与素材源片段失配。
  Phase 1 Clip source-range contract gate update: added `npm run audit:project-model-clip-source`, covering paired Clip `sourceStartMs/sourceEndMs`, source-range ordering, runtime `trackId/assetId/name` types, and legacy VideoEditor region provenance in the validator and aggregate audit so restored timeline clips do not drift away from their source media ranges.
- Phase 1 结构化验收入口更新：`npm run accept:phase1` 默认只读输出当前产品审计数量、UA-01 到 UA-08 的机器证据和真人动作；`npm run audit:phase1-acceptance-runner` 保护机器证据不能代替用户签字、UA-08 必须等待 UA-01 到 UA-07 全部显式验收的放行边界。
  Phase 1 structured acceptance entry update: `npm run accept:phase1` is read-only by default and reports current product-audit totals plus UA-01 through UA-08 machine evidence and hands-on actions; `npm run audit:phase1-acceptance-runner` guards that machine evidence cannot replace a user signature and UA-08 must wait for explicit acceptance of UA-01 through UA-07.
- Phase 1 外部条件验收入口更新：`npm run audit:transcription-acceptance-contract` 保护真实 App Bundle/TCC 身份、非空转录与字幕保存恢复合同；`npm run audit:share-production-lifecycle-contract` 保护生产分享默认离线、只读探测显式开启，以及写入必须双重确认、双测试账户和专用素材。
  Phase 1 external-condition acceptance entry update: `npm run audit:transcription-acceptance-contract` guards the real App Bundle/TCC identity, non-empty transcription, and subtitle save/restore contract; `npm run audit:share-production-lifecycle-contract` guards offline-by-default production sharing, explicit read-only probes, and write mode requiring double confirmation, two test accounts, and a dedicated fixture.
