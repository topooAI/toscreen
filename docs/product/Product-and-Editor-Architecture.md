...
| PH1-52 实机验收前预检包 / Hands-on acceptance handoff packet | ✅ Done | `npm run audit:phase1-handoff` 输出最新真实录制、proxy/audio/project sidecar、ProjectModel 恢复摘要、待验收 UA、逐项 `handsOnSteps`、组合后的 `acceptancePlan` 和建议运行命令，方便进入 Electron 实机验收。 / `npm run audit:phase1-handoff` reports the latest real recording, proxy/audio/project sidecar, ProjectModel restore summary, pending UA items, per-item `handsOnSteps`, the combined `acceptancePlan`, and suggested commands before Electron hands-on review. |
