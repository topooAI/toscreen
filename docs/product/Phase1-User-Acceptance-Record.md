...
| `npm run audit:timeline-range-zoom` | 检查无限时间轴的缩放与平移。 / Checks infinite-timeline zoom and pan. | 缩放 range 时左边缘必须固定；平移必须保留请求起点；0 点不可越界，右侧工作区不得由 Main Clip 或项目内容时长封顶。 / Range zoom must keep the left edge fixed; pan must preserve requested start; zero cannot drift left, and Main Clip or project content duration must not cap the right workspace. |
...
| `npm run audit:project-model-canvas-settings` | 检查 ProjectModel 画布、背景、导出质量和 legacy 动效字段。 / Checks ProjectModel canvas, background, export quality, and legacy effect fields. | aspect ratio、wallpaper、blur、padding、radius、shadow、crop、export quality 和 `motionBlurEnabled` 必须有明确类型和可渲染数值。 / Aspect ratio, wallpaper, blur, padding, radius, shadow, crop, export quality, and `motionBlurEnabled` must have explicit types and renderable values. |
| `npm run audit:project-model-entity-schema` | 检查 ProjectModel 顶层实体 schema。 / Checks the top-level ProjectModel entity schema. | Project、Asset、Track、Clip 的 id/name/timestamp/type 必须是可识别的受控字段，不能写入未知 asset/track/clip 类型。 / Project, Asset, Track, and Clip id/name/timestamp/type fields must stay controlled and cannot contain unknown asset/track/clip types. |
| `npm run audit:project-model-ui-source-schema` | 检查 UI Source 和 UI Element schema。 / Checks UI Source and UI Element schema. | UI source provider、source path、capturedAt、element role、selector 和 bounds 必须可验证，保证 UI-aware motion 不把录屏当成不可解释像素。 / UI source provider, source path, capturedAt, element role, selector, and bounds must be verifiable so UI-aware motion does not reduce recordings to unexplained pixels. |
| `npm run audit:project-model-clip-source` | 检查 Clip 源范围和 legacy 来源合同。 / Checks Clip source-range and legacy-provenance contracts. | `sourceStartMs/sourceEndMs` 必须成对、有限、非负且顺序正确；legacy 来源必须是受控的 VideoEditor region。 / `sourceStartMs/sourceEndMs` must be paired, finite, non-negative, and ordered; legacy provenance must be controlled VideoEditor region data. |
| `npm run audit:phase1-handoff` | 输出实机验收前预检包。 / Outputs the hands-on acceptance handoff packet. | 必须能找到最新真实录制、合法 ProjectModel sidecar、ProjectModel 本地素材文件检查 `assetFiles`、`editorRuntime`、待验收 UA 列表、`handsOnSteps`、`acceptancePlan` 和 `sceneMigration`；`acceptancePlan.machineEvidence` 必须引用真实 npm scripts。 / Must find the latest real recording, valid ProjectModel sidecar, ProjectModel local asset-file check `assetFiles`, `editorRuntime`, pending UA list, `handsOnSteps`, `acceptancePlan`, and `sceneMigration`; `acceptancePlan.machineEvidence` must reference real npm scripts. |
| `npm run audit:phase1-user-review-packet` | 输出用户评审入口包。 / Outputs the user review entry packet. | 必须合并模型能力、开放问题、最新真实录制、ProjectModel 恢复摘要、ProjectModel 本地素材文件检查 `assetFiles`、旧无 Scene sidecar 的 `sceneMigration` 预览和 UA 验收计划；它不能替代用户勾选验收。 / Must combine model capabilities, open questions, latest real recording, ProjectModel restore summary, ProjectModel local asset-file check `assetFiles`, `sceneMigration` preview for old scene-less sidecars, and UA acceptance plan; it cannot replace user check-off acceptance. |
