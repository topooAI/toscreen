...
| `npm run audit:timeline-lane-wrapping` | 检查时间轴可视分轨。 / Checks timeline visual lane wrapping. | 重叠 Annotation 必须自动分到不同可视 lane，贴边片段可复用同一 lane。 / Overlapping annotations must wrap onto separate visual lanes, while edge-touching clips may reuse one lane. |
