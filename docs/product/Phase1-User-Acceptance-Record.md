...
| `npm run audit:phase1-handoff` | 输出实机验收前预检包。 / Outputs the hands-on acceptance handoff packet. | 必须能找到最新真实录制、合法 ProjectModel sidecar、待验收 UA 列表、`handsOnSteps`、`acceptancePlan` 和 `sceneMigration`；`acceptancePlan.machineEvidence` 必须引用真实 npm scripts。 / Must find the latest real recording, valid ProjectModel sidecar, pending UA list, `handsOnSteps`, `acceptancePlan`, and `sceneMigration`; `acceptancePlan.machineEvidence` must reference real npm scripts. |
