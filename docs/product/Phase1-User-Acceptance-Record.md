...
| `npm run audit:project-model-default-scene` | 检查真实保存路径的默认 Scene 结构。 / Checks the default Scene structure on the real save path. | 非空 ProjectModel 必须生成覆盖工程总长、引用所有 clip 的默认 `demo` Scene。 / A non-empty ProjectModel must generate a full-duration default `demo` Scene that references every clip. |
