...
| RULE-04 | 片段选中样式 / Clip selected style | 选中态使用不改变尺寸的内发光样式。 / Selected clips should use inset glow without changing layout dimensions. | ❌ Not Complete | 当前 CSS 主要是改变背景色，还没有统一内发光标准。 / Current CSS changes background color; no inset glow standard exists yet. |
...
| CLIP-04 | 统一 6px 圆角 / Consistent 6px clip radius | 视觉检查并核对 CSS。 / Inspect all clip types visually and in CSS. | 所有片段卡片使用 6px 圆角。 / All clip cards use 6px radius. | ❌ Not Complete | `ItemGlass.module.css` |
| CLIP-05 | 选中态内发光 / Selected clip inset glow | 逐个选中各类片段。 / Select each clip type. | 选中态使用内发光/内描边，不改变片段尺寸。 / Selection uses inset glow/outline and does not change the clip size. | ❌ Not Complete | `ItemGlass.module.css` |
...
以下说法不要继续作为验收标准，除非先重写规则。  
Do not use these as acceptance standards without rewriting them.
...
| 所有片段已经是 6px 圆角。 / All clips already use 6px radius. | Incorrect / 不准确 | 当前片段 CSS 是 4px。 / Current clip CSS uses 4px. |
| 所有选中卡片都是纯内发光。 / All selected cards use pure inset glow. | Incorrect / 不准确 | 当前 selected 状态主要改变背景色。 / Current selected state changes background color. |
