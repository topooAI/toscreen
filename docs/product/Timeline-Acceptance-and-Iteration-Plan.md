...
| UA-04 Screen Studio 核心体验验收 / Screen Studio-grade UX review | CLIP-01 至 CLIP-06，VIDEO-01 至 VIDEO-06，AUDIO-01 / CLIP-01 through CLIP-06, VIDEO-01 through VIDEO-06, AUDIO-01 | 视觉和手感必须由 Electron 体验判断。 / Visual polish and interaction feel must be judged in Electron. |
...
| RULE-02 工程总长 / Project duration | `npm run audit:project-duration`, `npm run audit:timeline-duration-domains`, `npm run audit:preview-project-time`, `npm run audit:export-duration-render-settings` | 时间轴坐标域、播放控件、预览、导出都以工程总长合同为基础。 / Timeline coordinate domain, playback controls, preview, and export are based on the project-duration contract. |
| RULE-03 / CLIP-06 拉伸把手 / Resize handles | `npm run audit:timeline-resize-handles`, `npm run audit:electron-editor-runtime` | 所有 clip 类型的左右拉伸把手使用同一套竖向白色圆角小手柄；旧 SVG 端帽、横线、黄色把手和文本符号不会回归。 / All clip types use the same vertical white rounded resize handles; legacy SVG caps, horizontal dashes, yellow handles, and text glyphs cannot return. |
...
| CLIP-01 至 CLIP-05 片段基础样式 / Clip base style | `npm run audit:timeline-clip-style` | 片段 3px 微缝、标题左上角 6px 对齐、纯文本无 Lucide 图标、6px 圆角和内发光选中态被保护。 / The 3px clip gap, 6px top-left title alignment, text-only labels without Lucide icons, 6px radius, and inset selected glow are guarded. |
...
| VIDEO-04A 无限时间轴缩放与平移合同 / Infinite timeline zoom and pan contract | `npm run audit:timeline-range-zoom` | 缩放 range 时固定左边缘，平移时保留请求起点；0 点不可越界，右侧工作区不受 Main Clip 或项目内容长度封顶。 / Range zoom keeps the left edge fixed and pan preserves the requested start; zero cannot drift left, while the right workspace is not capped by Main Clip or project content duration. |
...
| AUDIO-01 波形底部显示 / Waveform bottom placement | `npm run audit:timeline-waveform-layout` | 音频波形必须限制在片段底部 45%，标题层保持左上角并位于波形之上，选中态音量包络只占下半区。 / Audio waveform rendering must stay confined to the bottom 45%; the title remains top-left above the waveform, and the selected volume envelope stays in the lower half. |
...
| VIDEO-02 物理 Trim 轨隐藏 / Physical Trim track hidden | `npm run audit:timeline-trim-row-hidden` | 独立 Trim Row 不得渲染；Trim item 只作为嵌套片段挂在 Main Track；主视频轨通过 `buildMainClipSegments` 展示保留源片段。 / A standalone Trim Row must not render; Trim items mount only as nested clips inside Main Track; the main video row uses `buildMainClipSegments` to show kept source segments. |
...
| ID | 验收项 / Acceptance Item | 验收方法 / Acceptance Method | 预期结果 / Expected Result | Current Status | Code Area |
|---|---|---|---|---|---|
| CLIP-01 | 3px 片段缝隙 / 3px clip gap | 将两个片段紧贴摆放。 / Place two clips adjacent to each other. | 片段之间保留精致微缝，不会视觉粘连。 / A precise small gap is visible; clips do not visually merge. | 🔒 Machine-Guarded | `Item.tsx`, `audit:timeline-clip-style` |
| CLIP-02 | 片段标题左上角对齐 / Clip title alignment | 检查 Video、Zoom、Audio、Annotation、Trim 标签。 / Inspect Video, Zoom, Audio, Annotation, and Trim labels. | 文本左上角对齐，顶部和左侧约 6px 内边距。 / Text is top-left aligned with 6px top and left padding. | 🔒 Machine-Guarded | `Item.tsx`, `audit:timeline-clip-style` |
| CLIP-03 | 片段内部无 Lucide 图标 / No inner Lucide icons inside clips | 检查所有片段类型。 / Inspect all clip types. | 片段主体只显示文本，不显示 Lucide 图标。 / Clip body shows text only, no Lucide icon inside the clip. | 🔒 Machine-Guarded | `Item.tsx`, `audit:timeline-clip-style` |
| CLIP-04 | 统一 6px 圆角 / Consistent 6px clip radius | 视觉检查并核对 CSS。 / Inspect all clip types visually and in CSS. | 所有片段卡片使用 6px 圆角。 / All clip cards use 6px radius. | 🔒 Machine-Guarded | `ItemGlass.module.css`, `audit:timeline-clip-style`, `audit:electron-editor-runtime` |
| CLIP-05 | 选中态内发光 / Selected clip inset glow | 逐个选中各类片段。 / Select each clip type. | 选中态使用内发光/内描边，不改变片段尺寸。 / Selection uses inset glow/outline and does not change the clip size. | 🔒 Machine-Guarded | `ItemGlass.module.css`, `audit:timeline-clip-style`, `audit:electron-editor-runtime` |
| CLIP-06 | 统一竖向拉伸把手 / Unified vertical resize handles | 悬浮或选中各类片段。 / Hover/select each clip type. | 左右两侧均显示与音频轨道一致的竖向白色圆角把手，不出现横线、括号状 SVG 端帽或额外装饰。 / Both edges show the same vertical white rounded handles as audio clips. No horizontal dash, bracket-like SVG cap, or extra decoration. | 🔒 Machine-Guarded | `Item.tsx`, `ItemGlass.module.css`, `audit:timeline-resize-handles` |
...
| VIDEO-02 | 物理 Trim 轨隐藏 / Physical Trim track hidden | 添加 Trim 区段后检查时间轴行，并运行机器门禁。 / Add Trim regions, inspect timeline rows, and run the machine gate. | Trim 被折叠进主视频表现中，不显示独立 Trim 轨道；嵌套 Trim 和主片段分段结构稳定。 / Trim is folded into main video representation with no separate visible Trim row; nested Trim and main-clip segmentation structure stay stable. | 🔒 Machine-Guarded | `TimelineEditor.tsx`, `audit:timeline-trim-row-hidden` |
...
| VIDEO-04A | 无限时间轴缩放与平移合同 / Infinite timeline zoom and pan contract | 模拟 zoom、pan、负起点、最小可视范围、最大 span，以及 1 小时和 24 小时后的工作区。 / Simulate zoom, pan, negative start, minimum visible range, maximum span, and workspace positions beyond one hour and 24 hours. | 缩放时左边缘保持稳定；平移不被误判为缩放；0 点不会漂进侧边栏；右侧不受内容时长封顶。 / Zoom keeps the left edge stable; pan is not mistaken for zoom; zero does not drift into the sidebar; the right side is not capped by content duration. | 🔒 Machine-Guarded | `TimelineWrapper.tsx`, `timelineRangeZoom.ts`, `audit:timeline-range-zoom` |
...
| VIDEO-06 | Main Track 与时间刻度解耦 / Main Track decoupled from timeline ticks | Main Track 比其他轨道短时观察刻度、播放头和播放控件总时长。 / Observe ticks, playhead, and playback-control total duration when Main Track is shorter than other tracks. | 时间刻度和播放控件继续显示到工程总时长；Main Track 片段只是其中一个片段，不定义全局结尾。 / Timeline ticks and playback controls continue to project duration; Main Track is only one clip and does not define the global end. | 🔒 Machine-Guarded | `TimelineEditor.tsx`, `PlaybackControls`, `audit:timeline-duration-domains` |

### P5：音频波形与边界 / Audio Waveform And Audio Boundaries

| ID | 验收项 / Acceptance Item | 验收方法 / Acceptance Method | 预期结果 / Expected Result | Current Status | Code Area |
|---|---|---|---|---|---|
| AUDIO-01 | 波形底部显示 / Waveform bottom placement | 检查音频片段，并运行机器门禁。 / Inspect audio clips and run the machine gate. | 波形限制在卡片底部 45%，不遮挡标题；标题层和音量包络层级稳定。 / Waveform is confined to the bottom 45% and does not cover the title; title and volume-envelope layering stay stable. | 🔒 Machine-Guarded | `Item.tsx`, `audit:timeline-waveform-layout` |
...
| 所有拉伸把手都是纯白竖条。 / All resize handles are pure vertical white handles. | Resolved / 已修正 | `audit:timeline-resize-handles` 现在保护统一竖向白色圆角小手柄，并禁止旧 SVG 端帽、横线、黄色把手和文本符号回归。 / `audit:timeline-resize-handles` now guards the unified vertical white rounded handles and prevents legacy SVG caps, horizontal dashes, yellow handles, and text glyphs from returning. |
...
- 新增 `audit:timeline-waveform-layout`，把 `AUDIO-01` 的波形底部 45%、标题左上角层级和选中音量包络下半区规则升级为机器门禁。
  Added `audit:timeline-waveform-layout`, upgrading `AUDIO-01` waveform bottom 45%, top-left title layering, and selected volume-envelope lower-half rules into a machine gate.
- 新增 `audit:timeline-trim-row-hidden`，把 `VIDEO-02` 的物理 Trim 轨隐藏、Trim 嵌套挂载到 Main Track 和主片段分段 helper 接线升级为机器门禁。
  Added `audit:timeline-trim-row-hidden`, upgrading `VIDEO-02` physical Trim-row hiding, nested Trim mounting inside Main Track, and main-clip segmentation helper wiring into a machine gate.
- 扩展 `audit:timeline-duration-domains`，把 `VIDEO-06` 的播放控件总时长接线纳入工程总长合同，防止 PlaybackControls 回退到源视频长度。
  Expanded `audit:timeline-duration-domains` to include `VIDEO-06` playback-control duration wiring in the project-duration contract, preventing PlaybackControls from falling back to source-video duration.
