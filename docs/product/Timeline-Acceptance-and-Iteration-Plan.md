...
| DRAG-03 / DRAG-04 可视分轨 / Visual lane wrapping | `npm run audit:timeline-lane-wrapping`, `npm run audit:project-model-lane-wrapping` | Timeline 可视层会把重叠 Zoom/Focus、Annotation、Audio 分到不同同类型 lane；ProjectModel 层也保护同类型 lane wrapping。 / Timeline visual layout wraps overlapping Zoom/Focus, annotation, and audio clips onto separate same-type lanes; ProjectModel also guards same-type lane wrapping. |
