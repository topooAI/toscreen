...
interface TimelineEditorProps {
  ...
  onFullScreenBindingChange: (isFull: boolean) => void;
  isPlaying: boolean;
...
export default function TimelineEditor({
  ...
  // Audio waveform
  const { peaks: audioPeaks } = useAudioWaveform(videoPath, 1000);
  ...
  const handleTimelineSeek = useCallback((timeInSeconds: number) => {
    if (!onSeek) return;
    const timelineMs = timeInSeconds * 1000;
    const sourceMs = isTrimTrackVisible ? timelineMs : mapEffectiveToSource(timelineMs);
    onSeek(sourceMs / 1000);
  }, [onSeek, isTrimTrackVisible, mapEffectiveToSource]);
  ...
  const handleAddTrim = useCallback(() => {
    ...
    const defaultDuration = Math.min(50, totalMs);
    if (defaultDuration <= 0) {
      return;
    }
    ...
    // Check if trim is inside any trim region
    const isOverlapping = sorted.some(region => startPos >= region.startMs && startPos < region.endMs);
    if (isOverlapping || gapToNext <= 0) {
    ...
    const actualDuration = Math.min(50, gapToNext);
    onTrimAdded({ start: startPos, end: startPos + actualDuration });
  }, [videoDuration, totalMs, currentTimeMs, trimRegions, onTrimAdded, activeDurationMs]);
  ...
  const timelineItems = useMemo<TimelineRenderItem[]>(() => {
    ...
    if (!isTrimTrackVisible) {
      const sortedTrimsForClips = [...trimRegions].sort((a, b) => a.startMs - b.startMs);
      let currentSourceStart = 0;
      sortedTrimsForClips.forEach((trim, index) => {
    ...
    // 如果 Trim 轨道不显示，那么就不能把 trims 传给底层库，否则底层库找不到对应的 Row 会报致命错误
    if (!isTrimTrackVisible) {
      return [...zooms, ...annotations, ...mainClips];
    }
    return [...zooms, ...trims, ...annotations, ...mainClips];
  }, [zoomRegions, trimRegions, annotationRegions, isTrimTrackVisible, mapSourceToEffective, totalMs, audioPeaks]);
  ...
  const handleItemSpanChange = useCallback((id: string, span: Span) => {
    ...
    } else if (id.startsWith('main-clip-')) {
      if (isTrimTrackVisible) return; // 磁吸模式下才允许调整片段边缘
      
      const sortedTrims = [...trimRegions].sort((a, b) => a.startMs - b.startMs);
      let prevTrim: TrimRegion | undefined, nextTrim: TrimRegion | undefined;
      let startSource = 0, endSource = totalMs;

      if (id === 'main-clip-final') {
        prevTrim = sortedTrims[sortedTrims.length - 1];
        startSource = prevTrim ? prevTrim.endMs : 0;
        endSource = totalMs;
      } else {
        const index = parseInt(id.replace('main-clip-', ''), 10);
        if (!isNaN(index)) {
          nextTrim = sortedTrims[index];
          prevTrim = index > 0 ? sortedTrims[index - 1] : undefined;
          startSource = prevTrim ? prevTrim.endMs : 0;
          endSource = nextTrim ? nextTrim.startMs : totalMs;
        }
      }

      // 根据是否是磁吸模式，提取真实的修改源时间
      // 如果是磁吸模式（isTrimTrackVisible 为 false），span 是有效时间，需要反解回源时间。
      // 因为在拖拽边缘时，我们期望拖拽的增量对应着源时间的增量。
      // targetSpan 就是正确的源时间（因为我们在外面已经算过了 targetSpan）
      const newSourceStart = targetSpan.start;
      const newSourceEnd = targetSpan.end;

      if (Math.abs(newSourceStart - startSource) > 1 && prevTrim) {
        // 左边缘被拖拽
        const delta = newSourceStart - startSource;
        const newEndMs = Math.min(prevTrim.endMs + delta, endSource);
        if (newEndMs > prevTrim.startMs) {
          onTrimSpanChange?.(prevTrim.id, { start: prevTrim.startMs, end: newEndMs });
        }
      } else if (Math.abs(newSourceEnd - endSource) > 1 && nextTrim) {
        // 右边缘被拖拽
        const delta = newSourceEnd - endSource;
        const newStartMs = Math.max(nextTrim.startMs + delta, startSource);
        if (newStartMs < nextTrim.endMs) {
          onTrimSpanChange?.(nextTrim.id, { start: newStartMs, end: nextTrim.endMs });
        }
      }
    }
  }, [zoomRegions, trimRegions, annotationRegions, onZoomSpanChange, onTrimSpanChange, onAnnotationSpanChange, isTrimTrackVisible, mapEffectiveToSource, totalMs]);
