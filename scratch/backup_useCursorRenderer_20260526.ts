  ...
  useEffect(() => {
    ...
    const ticker = (time: Ticker) => {
      ...
      if (isMock) {
        ...
        currentTimeMs = rawTimeMs % Math.max(1, mockDurationMs);
      } else {
        // For real recorded videos, clamp to the range of available mouse events
        currentTimeMs = Math.max(minTimeMs, Math.min(maxTimeMs, rawTimeMs));
      }
      ...
      if (isVisible) {
        ...
        if (container) {
          ...
          if (videoSprite && videoSprite.texture && videoSprite.texture.width > 0) {
            const baseScale = videoSprite.scale.x;
            const offsetX = videoSprite.position.x;
            const offsetY = videoSprite.position.y;
            const videoWidth = videoSprite.texture.width;
            const videoHeight = videoSprite.texture.height;

            // Map normalized coordinates (0-1) to local video container space (includes layout translation + scale)
            const localX = offsetX + currentX * videoWidth * baseScale;
            const localY = offsetY + currentY * videoHeight * baseScale;

            // Map local coordinates to global canvas space pixels (includes camera zoom/pan matrix!)
            const globalPos = container.toGlobal({ x: localX, y: localY });
            finalX = globalPos.x;
            finalY = globalPos.y;
          }
        }
